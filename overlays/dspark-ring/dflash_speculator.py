...
class DFlashSpeculator(DraftModelSpeculator):
    ...
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        self._ring_window: int = 0
        self._ring_inited: bool = False
    ...
    def _init_ring_state(self) -> None:
        """Ring-buffer draft KV + training capture state. Called from set_attn
        (after subclass __init__ has fixed num_query_per_req, e.g. DSpark's
        sample_from_anchor) so the mask width matches the real gamma."""
        if self._ring_inited:
            return
        self._ring_inited = True
        device = self.device
        ...
        if self._ring_window > 0:
            ...
            self._ring_drop_rejected = (
                _os.environ.get("VLLM_DSPARK_RING_DROP_REJECTED", "1") == "1"
            )
            self._ring_mask_excl_rejected = (
                _os.environ.get("VLLM_DSPARK_RING_MASK_EXCL_REJECTED", "1") == "1"
            )
            DSPARK_RING_CTX.ctx_positions = (
                torch.zeros(self.max_num_tokens, dtype=torch.long, device=device)
                if self._ring_drop_rejected else None
            )
    ...
    def set_attn(
        ...
        if self._ring_window > 0:
            # Ring mode: the draft owns no allocator KV group at all.
    ...
    def propose(
        ...
        # (0xdfi adaptive runner). DFlash drafts a whole block in one pass and
        # has no per-step loop to shorten, so the selected depth is unused here.
        num_speculative_tokens: int | None = None,
    ) -> torch.Tensor:
        self._init_ring_state()  # lazy: after subclass __init__, before any forward
        ...
        if self._capture_dir and not dummy_run and num_target_tokens > 0:
            ...
            if aux_hidden_states and self._capture_idx % self._capture_every == 0:
                try:
                    ...
                    if self._capture_idx == self._capture_every:
                        logger.info(
                            "DSpark capture active: %s (every %d steps)",
                            self._capture_dir, self._capture_every,
                        )
                except Exception as e:  # never break serving over capture
                    logger.warning_once("DSpark capture failed: %r", e)
        ...
        if self._ring_window > 0:
            ...
            prepare_dflash_inputs(
                ...
                self.max_model_len,  # "block size": every lookup -> row 0
            ...
            # Rejected draft tokens from the previous step arrive as context
            # too (the target ran them). In the paged path their KV sits at
            # the block's own positions and is overwritten by the query block
            # before attention; the ring would keep them visible. Send them
            # to the trash slot instead (position -> far negative => slot W).
            if self._ring_drop_rejected:
                tok = torch.arange(
                    num_target_tokens, device=lens.device, dtype=torch.long
                )
                seg_end = torch.repeat_interleave(
                    qsl[1:], lens, output_size=num_target_tokens
                )
                rej_start = seg_end - torch.repeat_interleave(
                    num_rejected[:num_r].to(torch.long), lens,
                    output_size=num_target_tokens,
                )
                DSPARK_RING_CTX.ctx_positions[:num_target_tokens] = torch.where(
                    tok >= rej_start,
                    torch.full_like(tok, -(1 << 40)),
                    self.context_positions[:num_target_tokens].to(torch.long),
                )
            ...
            if not (
                ...
                ctx_len = input_batch.seq_lens[:num_r].to(torch.long)
                if self._ring_mask_excl_rejected:
                    # seq_lens counts this step's block; the ring holds real
                    # context only up to the last accepted token. Slots past
                    # it are unwritten (zeros) at shallow depth or W-old.
                    ctx_len = ctx_len - num_rejected[:num_r].to(torch.long)
                valid = torch.clamp(ctx_len, min=0, max=w)
