...
class B12XSparseMLAScratchCaps:
    ...
    page_size: int = 64
    # nvfp4 pass-through metadata. The scratch LAYOUT does not depend on the KV
    # record width (only on head dims and kv_dtype, uint8 either way), so these
    # exist so callers can construct caps for the 368/432-byte record and so
    # ``workspace.scale_format`` is available to the decode API's
    # "explicit kwarg wins; otherwise the planned kv_cache_dtype supplies it"
    # resolution. They change no sizing.
