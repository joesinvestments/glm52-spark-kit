...
# --- fused DCP gather (decode) -------------------------------------------------
# At decode, the indexer's DCP top-k candidate all-gather and the MLA query-head
# all-gather (mla_attention.py, right before forward_mqa) are back-to-back on the
# same DCP group and data-independent. With VLLM_FUSED_DCP_GATHER=1 the indexer
# leaves its packed candidates here and MLA decode performs ONE all-gather over
# [q | candidates], then runs the stable top-k on the gathered candidates before
...
def dcp_topk_defer(packed: torch.Tensor, topk_indices: torch.Tensor, topk_tokens: int) -> None:
    if _PENDING_DCP_TOPK:
        raise RuntimeError("fused DCP gather: a deferred top-k candidate set was never consumed")
...
def dcp_topk_take():
    if not _PENDING_DCP_TOPK:
        return None
    d = dict(_PENDING_DCP_TOPK)
    _PENDING_DCP_TOPK.clear()
    return d
...
def _merge_dcp_topk_global(
    ...
    if defer and FUSED_DCP_GATHER:
        dcp_topk_defer(packed, topk_indices, topk_tokens)
        return
    gathered = get_dcp_group().all_gather(packed, dim=1)
    stable_topk_from_gathered_candidates_cutedsl(
        gathered, topk_tokens, out=topk_indices
    ...
    if has_decode:
        ...
        if decode_metadata.global_seq_lens is not None:
            _merge_dcp_topk_global(
                ...
                dcp_world_size,
                cp_kv_cache_interleave_size,
                # fused DCP gather: only when topk_indices is the live buffer view
                # (no unpack step follows) so the deferred finish lands in place.
                defer=not decode_metadata.requires_padding,
            )
