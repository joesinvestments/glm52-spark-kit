...
class SpeculativeConfig:
    ...
    # R17 port (ports/vllm/config/speculative.py): acceptance-length runtime
    # adaptation, ported from r9-payload/vllm/config/speculative.py. Builds
    # on top of 0.27.1's native num_speculative_tokens_per_batch_size (a
    # static batch-size -> depth schedule); this field controls the
    # separate adaptive/runtime-observed layer, absent upstream.
    ...
    def __post_init__(self):
        ...
        # R17 port: acceptance-length adaptation validation, ported verbatim
        # from r9-payload/vllm/config/speculative.py. Validated HERE, after
        # `self.method` has been fully derived from the draft model config
        # above -- a guard placed earlier would still read "draft_model" for
        # a medusa or mlp_speculator config and let it through.
        #
        # This is an ALLOW list, not a deny list: only these methods reach a
        # V2 speculator that INHERITS AutoRegressiveSpeculator.propose, which
        # is what carries the scheduler-selected depth into the draft loop.
        # Any method a future base adds is rejected until it is checked,
        # rather than silently accepted.
        #
        # `dflash` is deliberately excluded even though DFlashSpeculator.propose
        # accepts the argument: it overrides propose, ignores the depth, drafts
        # its whole block anyway and is then truncated by limit_draft_tokens --
        # the full cost with none of the saving.
        if self.adaptive_speculative_tokens_window is not None:
            supported_methods = {"mtp", "eagle", "eagle3"}
            if self.method not in supported_methods:
                raise ValueError(
                    "adaptive_speculative_tokens_window is only supported with "
                    "model-backed speculative decoding methods that draft one "
                    f"token per step ({', '.join(sorted(supported_methods))}); "
                    f"got '{self.method}'."
                )
            if (
