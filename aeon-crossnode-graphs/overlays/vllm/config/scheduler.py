...
class SchedulerConfig:
    ...
    # R17 port (ports/vllm/config/scheduler.py): decode-aware prefill
    # scheduling, ported from r9-payload/vllm/config/scheduler.py. 0.27.1's
    # native long_prefill_token_threshold only truncates a single long
    # prefill's chunk size; it does not budget aggregate prefill tokens
    # against active decode requests. That budgeting mechanism
    # (enable_decode_aware_prefill + the three budget fields below) was
    # absent upstream, so it is ported in full.
    ...
    def __post_init__(self, max_model_len: int, is_encoder_decoder: bool) -> None:
        ...
        # R17 port: decode-aware prefill validation, ported verbatim from
        # r9-payload/vllm/config/scheduler.py.
