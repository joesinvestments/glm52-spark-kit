...
# ---------------------------------------------------------------------------
# torch.library registration: ``torch.ops.vllm.nvfp4_ds_mla_write``.
#
# vLLM's kernel bar (registered schema + fake/meta impl + opcheck) so the writer
# is a first-class custom op: torch.compile can trace through it as an opaque
# mutating op, and CUDA-graph capture sees one op, not a python call.
# ``latent_scale`` is a 0-d fp32 tensor here (a Tensor, not a float, keeps the
# schema graph-safe: no scalar re-specialization per value).
# ---------------------------------------------------------------------------
OP_NAME = "nvfp4_ds_mla_write"
_OP_REGISTERED = False


def _op_impl(kv_cache: torch.Tensor, kv_c: torch.Tensor, k_pe: torch.Tensor,
             slot_mapping: torch.Tensor, latent_scale: torch.Tensor) -> None:
    write_to_kv_cache(kv_cache, kv_c, k_pe, slot_mapping, latent_scale=latent_scale)


def _op_fake(kv_cache: torch.Tensor, kv_c: torch.Tensor, k_pe: torch.Tensor,
             slot_mapping: torch.Tensor, latent_scale: torch.Tensor) -> None:
    # Pure in-place scatter: no outputs, shapes untouched. Shape checks only.
    torch._check(kv_c.shape[-1] == NOPE_DIM)
    torch._check(k_pe.shape[-1] == ROPE_DIM)
    torch._check(kv_c.shape[0] == k_pe.shape[0] == slot_mapping.shape[0])
    torch._check(kv_cache.dtype == torch.uint8)


def register_op() -> bool:
    """Register the op once. Safe to call repeatedly; False if vLLM is absent."""
    global _OP_REGISTERED
    if _OP_REGISTERED:
        return True
    try:
        from vllm.utils.torch_utils import direct_register_custom_op
    except Exception:  # pragma: no cover  (standalone use outside vLLM)
        return False
