...
class VllmConfig:
    ...
    def _maybe_override_dynamic_sd_cudagraph_mode(self) -> None:
        ...
        # R17 port (ports/vllm/config/vllm.py): the V2 full-CUDA-graph
        # exemption, ported from r9-payload/vllm/config/vllm.py.
        #
        # Downgrading ANY dynamic speculative decoding to PIECEWISE is
        # correct in general because the target verification length moves
        # at runtime, and a descriptor set captured for one uniform query
        # length cannot serve another. But that premise does not hold for
        # acceptance-length adaptation on the V2 runner: its
        # CudaGraphManager._init_candidates() captures a FULL decode
        # descriptor for every rung of the VLLM_ADAPTIVE_SPEC_DEPTHS ladder
        # at every request count up to max_num_seqs, and the scheduler can
        # only select depths from that same ladder (both read
        ...
        #   * batch-size schedule  -> still downgraded even on V2 (its K
        #                             comes from the batch size, not the
        #                             ladder, so the captured set does not
        #                             bound it).
        if (
    ...
    def _get_v2_model_runner_unsupported_features(self) -> list[str]:
        ...
        if speculative_config is not None:
            ...
            # R17 port: acceptance-length adaptation is supported by the V2
            # runner -- the scheduler-selected K is threaded through
            # ExecuteModelState to AutoRegressiveSpeculator.propose(), which
            # shortens the MTP draft loop. The static batch-size schedule
            # keeps the original restriction: it has not been verified under
            # V2 (its K comes from batch size, not the captured-graph
            # ladder), so it stays on the unsupported list even though
            # acceptance-length adaptation does not.
