...
OVERLAY_DIR="/var/tmp/fused-overlay"
NNODES=4
HEAD="${NODES[0]}"
...
ENVV=(
  ...
  -e "NCCL_IGNORE_CPU_AFFINITY=1" -e "NCCL_DEBUG=INFO" -e "NCCL_DEBUG_SUBSYS=INIT,NET" -e "NCCL_DEBUG_FILE=/var/log/nccl/nccl.%h.%p.log" -v /var/log/nccl:/var/log/nccl
...
KMOUNTS=(
  ...
  -v "$OVERLAY_DIR/scheduler_preempt_guard.py:/usr/local/lib/python3.12/dist-packages/vllm/v1/core/sched/scheduler.py:ro"
  ...
  -v "$OVERLAY_DIR/sparse_attn_indexer.py:$FM/layers/sparse_attn_indexer.py:ro"
