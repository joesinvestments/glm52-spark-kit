...
def _load_topk_input_from_row_base(
    ...
    if cutlass.const_expr(is_tiled):
        tile_size = Int64(block_q * block_k)
        k_tile_idx = Int32(0)
        k_local = Int32(0)
        ...
        value = Float32(
            input_tensor[
                row_base
                + Int64(k_tile_idx) * tile_size
                + Int64(k_local)
            ]
        )
    else:
        value = Float32(input_tensor[row_base + Int64(logical_k)])
    return value
...
def _to_kernel_tensor(tensor, dtype, *, assumed_align=16):
    dynamic_layout = _should_mark_layout_dynamic(tensor)
...
def _should_mark_layout_dynamic(tensor: torch.Tensor) -> bool:
    if tensor.ndim == 1 and int(tensor.numel()) > _SIGNED_INT32_MAX:
        return False
    return True
...
def _flat_tensor_compile_key(name, tensor, *, dynamic=False):
    ...
    if dynamic and _should_mark_layout_dynamic(flat):
        dims = (DimKey.dynamic(),)
    else:
        dims = (DimKey.exact(int(flat.shape[0])),)
    return tensor_compile_fact(name, flat, dims=dims)


def _checked_int32(value: int, *, name: str) -> Int32:
    value = int(value)
    if value < _SIGNED_INT32_MIN or value > _SIGNED_INT32_MAX:
        raise OverflowError(
            f"{name}={value} is outside signed int32 range "
            f"[{_SIGNED_INT32_MIN}, {_SIGNED_INT32_MAX}]"
        )
    return Int32(value)


def _checked_nonnegative_int32(value: int, *, name: str) -> Int32:
    value = int(value)
    if value < 0:
        raise ValueError(f"{name} must be non-negative, got {value}")
    return _checked_int32(value, name=name)
...
class SparseNSATiledTopkKernel:
    ...
    def kernel(
        ...
        row_base = Int64(data_bid) * input_stride
        if cutlass.const_expr(self.is_tiled):
            block_q_i = Int32(self.block_q)
            block_k_i = Int32(self.block_k)
            tile_size = Int64(self.block_q * self.block_k)
            q_tile_idx = data_bid // block_q_i
            q_local = data_bid - q_tile_idx * block_q_i
            row_base = (
                Int64(q_tile_idx) * Int64(num_k_tiles) * tile_size
                + Int64(tile_k_offset) * tile_size
                + Int64(q_local) * Int64(block_k_i)
            )
...
def run_tiled_topk(
    *,
    ...
    args = (
        ...
        _checked_nonnegative_int32(num_q_rows * extent_splits, name="batch_size"),
        input_stride,
        _checked_nonnegative_int32(num_k_tiles, name="num_k_tiles"),
        _checked_nonnegative_int32(tile_k_offset, name="tile_k_offset"),
        _checked_nonnegative_int32(block_q, name="block_q"),
        _checked_nonnegative_int32(block_k, name="block_k"),
        _checked_nonnegative_int32(topk, name="topk"),
        _checked_nonnegative_int32(input_index_offset, name="input_index_offset"),
        _checked_nonnegative_int32(input_extent, name="input_extent"),
        _checked_nonnegative_int32(output_index_offset, name="output_index_offset"),
        _checked_nonnegative_int32(output_page_size, name="output_page_size"),
        _checked_nonnegative_int32(output_row_stride, name="output_row_stride"),
        _checked_nonnegative_int32(output_row_base, name="output_row_base"),
        current_cuda_stream(),
    )
    cache_key = (
...
def run_row_topk(
    *,
    ...
    args = (
        ...
        _checked_nonnegative_int32(num_q_rows, name="batch_size"),
        Int64(width),
        _checked_nonnegative_int32(0, name="num_k_tiles"),
        _checked_nonnegative_int32(0, name="tile_k_offset"),
        _checked_nonnegative_int32(1, name="block_q"),
        _checked_nonnegative_int32(1, name="block_k"),
        _checked_nonnegative_int32(topk, name="topk"),
        _checked_nonnegative_int32(0, name="input_index_offset"),
        _checked_nonnegative_int32(width, name="input_extent"),
        _checked_nonnegative_int32(output_index_offset, name="output_index_offset"),
        _checked_nonnegative_int32(1, name="output_page_size"),
        _checked_nonnegative_int32(1, name="output_row_stride"),
        _checked_nonnegative_int32(0, name="output_row_base"),
        current_cuda_stream(),
    )
    cache_key = (
