...
# R17 port: adaptive/runtime acceptance-length depth control, ported from
# r9-payload (absent from 0.27.1 upstream; builds on top of the native
# dynamic/utils.py static batch-size schedule lookup below).
...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        # R17 port (ports/vllm/v1/core/sched/scheduler.py): decode-aware
        # prefill scheduling state, ported from r9-payload's scheduler.py.
        # Absent from 0.27.1 upstream, whose long_prefill_token_threshold
        # only truncates a single request's chunk size rather than budgeting
        # aggregate prefill tokens against active decode requests.
        ...
        # Liveness counters for the long-prefill gate: how often a running
        # long prefill lost its turn, and how often a selected turn produced
        # no scheduled work (see _debit_unused_long_prefill_turns).
        self.long_prefill_gate_skips = 0
        self.long_prefill_wasted_turns = 0
        ...
        # R17 port: AcceptanceLengthController wiring, ported from
        # r9-payload's scheduler.py __init__. min()-composes with the
        # native dynamic_sd_lookup (batch-size schedule) at selection time
        # -- see the num_spec_tokens_to_schedule computation below.
        self.acceptance_length_controller: AcceptanceLengthController | None = None
        # The effective depth ladder, resolved once from
        ...
        # R17 port: MTP instrumentation, ported from r9-payload's
        # scheduler.py __init__. Every field is derived from CPU-side
        # counts the scheduler already holds after verification
        # (scheduled_spec_decode_tokens and the sampled-token lists). No
        # forward pass, no device read, no synchronization is added for
        # telemetry. Emits a MTP_WINDOW_JSON log line every
        # mtp_instrument_window steps when VLLM_MTP_INSTRUMENT is truthy.
    ...
    # ── R17 port: decode-aware prefill selection ────────────────────────
    # Ported from r9-payload's scheduler.py. Absent from 0.27.1 upstream,
    # whose only related mechanism (long_prefill_token_threshold) truncates
    # a single request's chunk size but does not budget aggregate prefill
    # tokens against active decode requests, nor gate how many long
    # prefills run per step.
    ...
    def _select_long_prefill_request_ids(self) -> set[str]:
        ...
        running_candidates: list[Request] = []
        waiting_candidates: list[Request] = []
        seen: set[str] = set()
        for request in self.running:
            if self._is_long_prefill_request(request):
                seen.add(request.request_id)
                running_candidates.append(request)
        for request in itertools.chain(self.waiting, self.skipped_waiting):
            if request.request_id in seen:
                continue
            if self._is_long_prefill_request(request):
                seen.add(request.request_id)
                waiting_candidates.append(request)
        ...
        def by_age(requests: list[Request]) -> list[Request]:
            return sorted(
                requests,
                key=lambda request: (
        ...
        # The waiting loop cannot admit anything once the running queue is at
        # `max_num_seqs`: it breaks on capacity before it ever reaches the
        # long-prefill gate. A waiting request selected in that state therefore
        # holds a turn it cannot use, while every running long prefill is skipped
        # as unselected -- a step that schedules nothing, repeated forever
        # because a skipped request's `_long_prefill_last_scheduled` never ages.
        # So only consider as many waiting candidates as there are sequence slots
        # to admit them into; at full capacity that is none, which leaves the
        # turn with the running prefills that can actually use it.
        #
        # This is a no-op whenever `admittable >= max_long_prefills_per_step`:
        # the selection keeps at most that many entries, so the only waiting
        # candidates that could have been selected are the best `admittable` of
        # them, and those are exactly the ones kept here.
        #
        # `admittable` is a selection-time snapshot: a mid-step preemption can
        # invalidate it, but then the waiting loop is skipped for the whole step
        # (it runs only when `not preempted_reqs`), and the wasted turn is aged
        # by `_debit_unused_long_prefill_turns` at the end of the step.
        admittable = max(0, self.max_num_running_reqs - len(self.running))
        candidates = list(running_candidates)
        if admittable > 0 and waiting_candidates:
            keep = {
                request.request_id
                for request in by_age(waiting_candidates)[:admittable]
            }
            # Kept in queue order, so ties break exactly as they did before.
            candidates += [r for r in waiting_candidates if r.request_id in keep]

        ordered = by_age(candidates)
        return {
            request.request_id for request in ordered[: self.max_long_prefills_per_step]
        }

    def _debit_unused_long_prefill_turns(
        self,
        selected_long_prefill_ids: set[str],
        num_scheduled_tokens: dict[str, int],
        defer_prefills: bool,
    ) -> None:
        """Age a long-prefill turn that produced no work.

        `_long_prefill_last_scheduled` is otherwise written only for requests that
        were actually scheduled, so a selected request that could not be used
        never ages and wins the same turn on every following step. Charging it the
        current step bounds the rotation: consecutive steps hand the turn to each
        candidate in turn instead of one unusable request holding it forever.

        Steps on which policy deliberately issued no prefill work at all are not
        charged -- no candidate could have run, so ageing them would only churn
        the ordering.
        """
        if not self.enable_decode_aware_prefill or not selected_long_prefill_ids:
            return
        if defer_prefills or self._pause_state == PauseState.PAUSED_ALL:
            return
        for request_id in selected_long_prefill_ids:
            if request_id not in num_scheduled_tokens:
                self.long_prefill_wasted_turns += 1
                self._long_prefill_last_scheduled[request_id] = self.current_step

    # ── R17 port: MTP instrumentation ───────────────────────────────────
    # Ported from r9-payload's scheduler.py.
    ...
    def schedule(self, throttle_prefills: bool = False) -> SchedulerOutput:
        ...
        # R17 port: decode-aware prefill budgeting, ported from
        # r9-payload's scheduler.py.
        ...
        # Running long prefills skipped this step purely because they lost the
        # long-prefill turn. Counted so a step that schedules nothing while such
        # requests exist can be reported instead of passing silently.
        gate_skipped_running = 0
        ...
        while req_index < len(self.running) and token_budget > 0:
            ...
            # R17 port: decode-aware prefill gating, ported from
            # r9-payload's scheduler.py.
            ...
            if (
                ...
                gate_skipped_running += 1
                self.long_prefill_gate_skips += 1
                req_index += 1
                continue
            ...
            # R17 port: decode-aware prefill bookkeeping, ported from
            # r9-payload's scheduler.py.
            if self.enable_decode_aware_prefill and is_prefill:
        ...
        if not preempted_reqs and self._pause_state == PauseState.UNPAUSED:
            ...
            while (self.waiting or self.skipped_waiting) and token_budget > 0:
                ...
                if load_kv_async:
                    ...
                    if not self.enable_decode_aware_prefill and 0 < threshold < num_new_tokens:
                    ...
                    num_new_tokens = min(num_new_tokens, token_budget)
                    # R17 port: decode-aware prefill budget cap, ported from
                    # r9-payload's scheduler.py. NOTE (UNSURE, see
                    # PORTS_SCHED_NOTES.md): the long-prefill *gate*
                    # (selected_long_prefill_ids admission from the running
                    # loop) is intentionally NOT re-applied here -- this
                    # waiting-queue loop's control flow (pop/break/continue
                    # on `request_queue`) differs enough from r9-payload's
                    # that gating a candidate could stall the queue. Only
                    # the aggregate token budget and bookkeeping are ported;
                    # the gate remains running-queue-only in this port.
                    if self.enable_decode_aware_prefill:
        ...
        #
        # R17 port (ports/vllm/v1/core/sched/scheduler.py): AcceptanceLengthController
        # integration, ported from r9-payload's scheduler.py. Composed via
        # min() with the native batch-size dynamic_sd_lookup: whichever
        # layer wants FEWER speculative tokens for this step wins, so the
        # acceptance-length adaptation can only ever shrink K relative to
        # the static schedule, never grow past what the batch-size schedule
        # allows.
        ...
        # R17 port: decode-aware prefill turn ageing, ported from
        # r9-payload's scheduler.py.
        self._debit_unused_long_prefill_turns(
            selected_long_prefill_ids, num_scheduled_tokens, defer_prefills
        )
    ...
    def update_from_output(
        ...
        # R17 port: MTP instrumentation + AcceptanceLengthController sample
        # accumulation, ported from r9-payload's scheduler.py. Per-position
        # eligible/accepted counts and the (draft, accepted, computed,
        # output) samples are CPU-side counts derived from the same values
        # already read below; observe_batch() is called once, after the
        # loop, so the controller and the instrumentation window's samples
        # are derived from the same CPU-side counts.
        ...
        for req_id, num_tokens_scheduled in num_scheduled_tokens.items():
            ...
            # R17 port: req_id_to_index KeyError fix.
            #
            # Root cause: `request is None or request.is_finished()` above
            # only guards against a request that has fully finished (or been
            # aborted) by the time this output is processed. It does NOT
            # cover a request that is merely `output_is_stale` (preempted
            # mid-flight, see `_preempt_request` / `num_stale_output_tokens`)
            # with `drop_stale_output=False`: that request keeps receiving
            # its stale output share here, but the *model runner* that
            # produced `model_runner_output` for THIS step already evicted
            # it from its active batch when it was preempted -- the runner's
            # `req_id_to_index` is built from its own current batch, not
            # from `self.requests`. Under async scheduling the preemption on
            # the scheduler side can land before the in-flight step's output
            # comes back, so `req_id` is live in `self.requests` (not
            # finished) yet absent from `req_id_to_index`: a bare
            # `req_id_to_index[req_id]` KeyErrors. This is most visible with
            # nvfp4 + a quantized draft model because that combination's
            # larger per-step KV/draft footprint increases preemption
            # frequency, but the race applies to any async-scheduling
            # configuration.
            #
            # Fix: look the index up with `.get()` and skip this request's
            # output for the step (its state already reflects the stale
            # share drained above) instead of crashing the scheduler on a
            # transient, expected race. This is a targeted guard, not a
            # bare try/except -- it only swallows the one diagnosed case
            # (a request absent from this step's index) and logs it so the
            # rate is observable rather than silent.
            req_index = model_runner_output.req_id_to_index.get(req_id)
            if req_index is None:
                logger.debug(
                    "update_from_output: request %s is tracked "
                    "(self.requests) but absent from this step's "
                    "req_id_to_index (output_is_stale=%s, "
                    "drop_stale_output=%s); skipping its output for this "
                    "step -- most likely preempted and evicted from the "
                    "model runner's active batch between schedule() and "
                    "this async output landing.",
                    req_id,
                    output_is_stale,
            ...
            if scheduled_spec_token_ids and (
                ...
                # R17 port: MTP instrumentation + acceptance-length sample
                # collection, ported from r9-payload's scheduler.py. Gated
                # on `not output_is_stale` for the same reason the R17
                # rejection accounting above is gated: a window that counted
                # a stale frame would misreport the very acceptance ratio
                # the adaptive controller decides on.
        ...
        # R17 port: AcceptanceLengthController.observe_batch() +
        # MTP_WINDOW_JSON emission, ported from r9-payload's scheduler.py.
        #
        ...
        # `num_spec_tokens_to_schedule` is the field 0.27.1's schedule()
        # always sets (falling back to the static self.num_spec_tokens for
        # synthetic/warmup outputs that bypass schedule() and default it to
        # 0).
        active_num_spec_tokens = (
            scheduler_output.num_spec_tokens_to_schedule or self.num_spec_tokens
        )
        ...
        mtp_update = None
        if acceptance_length_controller is not None:
            mtp_update = acceptance_length_controller.observe_batch(
            ...
            if (
                mtp_update is not None
                and mtp_update.previous_num_spec_tokens != mtp_update.num_spec_tokens
            ):
                logger.info(
                    ...
                    mtp_update.previous_num_spec_tokens,
                    mtp_update.num_spec_tokens,
                    mtp_update.mean_num_accepted_tokens,
                    mtp_update.mean_num_draft_tokens,
                    mtp_update.observation_window,
                    mtp_update.decision_reason,
                    mtp_update.tail_gain_23,
                    mtp_update.position_4_gain,
                    ",".join(
                        f"{rate:.2f}"
                        for rate in mtp_update.position_conditional_acceptance
                    ),
                )

        self._record_mtp_instrumentation(mtp_samples, mtp_update, active_num_spec_tokens)
