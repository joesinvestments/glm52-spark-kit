...
def _nvfp4_split_data_scale(
    ...
    """Split one NVFP4 KV side into (data, scale) views — layout-agnostic.

    ``kv_side`` is a uint8 tensor whose LAST dim is
    ``full_dim = data_dim + scale_dim``, physically ``[data | scale]`` per
    (page, head, token).

    AEON: we deliberately use plain slicing instead of manual
    ``as_strided`` offset arithmetic. Upstream's ``nvfp4_split_data_scale``
    derives inner strides as ``stride(i) * data_dim // full_dim`` and offsets
    the scale view by ``dim_1 * dim_2 * data_dim``, which assumes each page is
    physically ``[all data | all scale]``. That only holds under **HND**;
    under **NHD** (the DGX Spark / GB10 default, since FlashInfer forces HND
    only when ``capability.major == 10``) the derived views MUTUALLY OVERLAP
    and silently corrupt KV with no error. Slicing lets PyTorch derive strides
    and offsets from the real layout, so the two views are disjoint by
    construction under any permutation. See ``tests/aeon/test_nvfp4_kv_views.py``.
    """
    full_dim = kv_side.shape[-1]
    data_dim = full_dim * 8 // 9
    data = kv_side[..., :data_dim]
    scale = kv_side[..., data_dim:].view(torch.float8_e4m3fn)
    return data, scale
...
def nvfp4_split_data_scale(
    ...
    """Upstream-compatible alias (see ``_nvfp4_split_data_scale``)."""
    return _nvfp4_split_data_scale(kv_side)
...
def nvfp4_kv_cache_split_views(kv_cache: torch.Tensor) -> tuple[tuple, tuple]:
    ...
    Accepts a 4D single-side tensor ``(num_pages, dim_2, dim_3, full_dim)``
    or a 5D ``(num_pages, 2, dim_2, dim_3, full_dim)`` tensor.
    Each KV side is self-contained (data followed by its scale).

    Returns ``(data...), (scale...)`` in the same dim order as the input.
    """
    if kv_cache.dim() == 4:
