...
| **node protection** | `launch/node_protect.sh` | SBSA hardware watchdog daemon (reboot on load/free-memory failure), earlyoom, sshd/dockerd OOM immunity, `plymouth-quit-wait` masked. Replaces the power button when a load thrashes the box. | live on all four nodes |
