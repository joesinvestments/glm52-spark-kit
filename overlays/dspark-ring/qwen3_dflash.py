...
def _dspark_ring_window() -> int:
    ...
    Port of bird/vllm-lil@dspark-ring-1m-20260711 (Apache-2.0) onto v0.27.0.
    """
    import os
...
class _DSparkRingCtx:
    ...
    def __init__(self) -> None:
        ...
        # [num_ctx_tokens] ingest positions with rejected tokens -> trash
        self.ctx_positions: torch.Tensor | None = None
...
class _DFlashDraftAttention(Attention):
    """Draft attention that owns NO allocator KV in ring mode (spec None):
    the ring buffers in DFlashQwen3Attention hold the window instead, so the
    draft never registers with the paged allocator and DCP sharding does not
    apply to it."""
...
class DFlashQwen3Attention(nn.Module):
    ...
    def __init__(
        ...
        self.attn = _DFlashDraftAttention(
            self.num_heads,
            self.head_dim,
            self.scaling,
...
class DFlashQwen3Model(nn.Module):
    ...
    def precompute_and_store_context_kv(
        ...
        if _dspark_ring_window() > 0:
            if ring_ctx.ctx_rows is None:
                return  # dummy run: no cache writes
            rows = ring_ctx.ctx_rows[:num_ctx]
            ing_pos = context_positions
            if ring_ctx.ctx_positions is not None:
                # rejected-token positions redirected to the trash slot
                ing_pos = ring_ctx.ctx_positions[:num_ctx]
            for i in range(L):
                self._ring_layers[i].ring_ingest(
                    all_k_final[i], all_v[i], rows, ing_pos
                )
            return
