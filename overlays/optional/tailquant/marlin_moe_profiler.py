...
# [glm52-collab][tailquant] Router-frequency profiler v2 - CAPTURE-SAFE.
# Hot path: preallocated buffers + index_add_ only. No host sync, no allocs
# after first warmup call, no I/O. Dump at process exit via atexit.
_TQ = {"dir": None, "buf": None, "ones": None, "tokens": 0, "key": None}
...
def _tq_record(owner, topk_ids):
    ...
    if d is None:
        import os as _os
        _TQ["dir"] = d = _os.environ.get("VLLM_TAILQUANT_PROFILE_DIR") or ""
    if not d or topk_ids is None or topk_ids.numel() == 0:
        return
    try:
        if _TQ["buf"] is None:
            n = max(256, int(topk_ids.shape[-1]))
            _TQ["buf"] = torch.zeros(n, dtype=torch.long, device=topk_ids.device)
            _TQ["ones"] = torch.ones_like(topk_ids, dtype=torch.long).reshape(-1)
            _TQ["key"] = getattr(owner, "prefix", None) or ("inst%d" % id(owner))
        buf = _TQ["buf"]
        flat = topk_ids.reshape(-1).to(torch.long)
        valid = flat[flat < buf.numel()]
        buf.index_add_(0, valid, _TQ["ones"][: valid.numel()])
    except Exception:
        pass


import atexit as _tq_atexit


def _tq_dump():
    d = _TQ.get("dir")
    if not d or _TQ.get("buf") is None:
        return
    try:
        ...
        key = str(_TQ.get("key", "layer")).replace("/", "_")
        _json.dump({key: _TQ["buf"].detach().cpu().tolist()},
                   open(out / ("router_hist_" + key + ".json"), "w"))
    except Exception:
        pass


_tq_atexit.register(_tq_dump)
