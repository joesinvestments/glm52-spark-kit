...
class EngineArgs:
    ...
    # R17 port: decode-aware prefill CLI-facing fields, ported from
    # r9-payload/vllm/engine/arg_utils.py (absent from 0.27.1 upstream).
    enable_decode_aware_prefill: bool = SchedulerConfig.enable_decode_aware_prefill
    decode_prefill_token_budget: int = SchedulerConfig.decode_prefill_token_budget
    idle_prefill_token_budget: int = SchedulerConfig.idle_prefill_token_budget
    max_long_prefills_per_step: int = SchedulerConfig.max_long_prefills_per_step
    ...
    def add_cli_args(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
        ...
        # R17 port: decode-aware prefill CLI flags, ported from
        # r9-payload/vllm/engine/arg_utils.py.
        scheduler_group.add_argument(
            "--enable-decode-aware-prefill",
            **scheduler_kwargs["enable_decode_aware_prefill"],
        )
        scheduler_group.add_argument(
            "--decode-prefill-token-budget",
            **scheduler_kwargs["decode_prefill_token_budget"],
        )
        scheduler_group.add_argument(
            "--idle-prefill-token-budget",
            **scheduler_kwargs["idle_prefill_token_budget"],
        )
        scheduler_group.add_argument(
            "--max-long-prefills-per-step",
            **scheduler_kwargs["max_long_prefills_per_step"],
        )
    ...
    def create_engine_config(
        ...
        scheduler_config = SchedulerConfig(
            ...
            enable_decode_aware_prefill=self.enable_decode_aware_prefill,
            decode_prefill_token_budget=self.decode_prefill_token_budget,
            idle_prefill_token_budget=self.idle_prefill_token_budget,
            max_long_prefills_per_step=self.max_long_prefills_per_step,
