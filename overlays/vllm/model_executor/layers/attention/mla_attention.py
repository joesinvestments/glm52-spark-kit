...
# --- Build A (R17): optional Triton kernel for the two MLA-absorb decode
# bmms below (BMM1 q_nope@W_UK_T, BMM2 attn_out@W_UV). Enabled only with
# VLLM_BUILDA_BMM=1; falls back to torch.bmm outside the verified envelope.
# See vllm/model_executor/layers/attention/builda_bmm_v0.py (staged from
# the private campaign integration tree; local path redacted for publication).
try:
    from vllm.model_executor.layers.attention.builda_bmm_v0 import (
        builda_bmm as _builda_bmm,
        builda_bmm_enabled as _builda_bmm_enabled,
    )
except Exception:  # kernel module not staged in this image

    def _builda_bmm_enabled() -> bool:
        return False
...
class MLAAttention(nn.Module, AttentionLayerBase):
    ...
    def forward_impl(
        ...
        if num_mqa_tokens > 0:
            ...
            if self.is_aiter_triton_fp4_bmm_enabled:
                ...
                if _builda_bmm_enabled():
                    _builda_bmm(mqa_q_nope, W_UK_T, mqa_ql_nope)  # Build A BMM1
                else:
            ...
            if self.impl.dcp_world_size > 1:
                if self.use_pcp:
                    ...
                    if isinstance(mqa_q, tuple):
                        ...
                        mqa_q = torch.cat(mqa_q, dim=-1)
                    if not qrep_decode:
                        # mqa_q do allgather in head dim. Fused DCP gather: if the
                        # sparse indexer deferred its top-k candidate gather for this
                        # layer, do ONE all-gather over [q | candidates] and finish the
                        # top-k here (byte-identical to two gathers, one collective).
                        from vllm.model_executor.layers.sparse_attn_indexer import (
                            dcp_topk_take, dcp_topk_finish)
                        _pending = dcp_topk_take()
                        if _pending is not None:
                            _packed = _pending["packed"]              # (B, topk, 2) fp32
                            _B = mqa_q.shape[0]
                            assert _packed.shape[0] == _B, (
                                f"fused DCP gather: batch mismatch q={_B} cand={_packed.shape[0]}")
                            _qh = mqa_q.shape[1]; _qd = mqa_q.shape[2]
                            _pk = _packed.shape[1]
                            _q_flat = mqa_q.reshape(_B, _qh * _qd)
                            _p_flat = _packed.reshape(_B, _pk * 2)
                            # dtypes differ (bf16 q, fp32 candidates): gather raw bytes.
                            _qb = _q_flat.contiguous().view(torch.uint8)
                            _pb = _p_flat.contiguous().view(torch.uint8)
                            _buf = torch.cat([_qb, _pb], dim=1)          # (B, qbytes + pbytes)
                            _g = get_dcp_group().all_gather(_buf, dim=1)   # (B, W*(qbytes+pbytes))
                            _W = get_dcp_group().world_size
                            _qbytes = _qb.shape[1]; _pbytes = _pb.shape[1]
    ...
    def _v_up_proj(self, x: torch.Tensor, out: torch.Tensor):
        ...
        if self.is_aiter_triton_fp4_bmm_enabled:
            ...
            # Multiply + Transpose (N, B, L) x (N, L, V)->(N, B, V)->(B, N, V)
            if _builda_bmm_enabled():
                _builda_bmm(x, self.W_UV, out.transpose(0, 1))  # Build A BMM2
            else:
